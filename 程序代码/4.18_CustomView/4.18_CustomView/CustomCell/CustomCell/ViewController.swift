...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 1 {
            ...
            cell?.price.text = "¥78"
            cell?.typeTitle.text = "100元代金卷1张，可叠加"
        }
        if indexPath.row == 2 {
            cell?.myImageView.image = UIImage(named: "tamikafei")
            cell?.myTitle.text = "塔米咖啡"
            cell?.image1.image = UIImage(named: "juan")
            cell?.subTitle.text = "咖啡厅"
            cell?.price.text = "¥7.9"
            cell?.typeTitle.text = "10元代金卷1张，可叠加"
        }
        if indexPath.row == 3 {
            cell?.myImageView.image = UIImage(named: "kanglixibing")
            cell?.myTitle.text = "康力西饼"
            cell?.image1.image = UIImage(named: "tuan")
            cell?.subTitle.text = "蛋糕"
            cell?.price.text = "¥88"
            cell?.typeTitle.text = "蛋糕5选1，约8寸，圆形"
        }
        if indexPath.row == 4 {
            cell?.myImageView.image = UIImage(named: "xiangcuncaiguan")
            cell?.myTitle.text = "乡村菜馆"
            cell?.image1.image = UIImage(named: "tuan")
            cell?.subTitle.text = "农家菜"
            cell?.price.text = "¥85"
            cell?.typeTitle.text = "100元代金卷1张，可叠加"
        }
        return cell!
